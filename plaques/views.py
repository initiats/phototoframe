...
def shop(request):
    ...
    customizations = ProductCustomization.objects.filter(product=product).prefetch_related(
        "thickness", "material", "fastening", "dimension", "textzone_set"
    )
    ...
    for c in customizations:
        configs.append({
            ...
            # ADD THIS LINE:
            "text_zones": [{"text": z.label, "x": float(z.x), "y": float(z.y)} for z in c.textzone_set.all()]
        })
...
def product_customize_save(request, pk):
    ...
    TextZone = apps.get_model('plaques', 'TextZone')  # Gets model by string name
    ...
    if request.method == "POST":
        ...
        for i in range(len(texts)):
            if texts[i].strip():
                TextZone.objects.create(
                    customization=custom,
                    label=texts[i],
                    x=tx[i] if i < len(tx) else 0,
                    y=ty[i] if i < len(ty) else 0,
                    w=tw[i] if i < len(tw) else 0,
                    h=th[i] if i < len(th) else 0
                )
...
def customization_edit(request, pk):
    from django.apps import apps
    TextZone = apps.get_model('plaques', 'TextZone')
    ...
    if request.method == "POST":
        ...
        # --- UPDATE TEXT ZONES ---
        edit_custom.textzone_set.all().delete()
        texts = request.POST.getlist("custom_text[]")
        tx = request.POST.getlist("tx[]")
        ty = request.POST.getlist("ty[]")
        tw = request.POST.getlist("tw[]")
