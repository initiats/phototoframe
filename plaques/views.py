...
def index(request):
    ...
    # 4. Fetch Accessories for the home page (First 4 items)
    accessories = Accessory.objects.all().order_by('-created_at')[:4]

    # 5. Fetch Collections for the home page
    collections = Collection.objects.all().order_by('-created_at')[:4]

    # 6. Return everything in ONE dictionary
    return render(request, "index.html", {
        "banners": banners,
        "categories": categories,
        "products": products,
        "accessories": accessories,  # Added this
        "collections": collections   # Added this
    })
...
from .models import Product, Category, ProductCustomization, PlaqueShape, Thickness, Material, Dimension
...
def shop(request):
    ...
    for c in customizations:
        configs.append({
            "id": c.id,
            "base_shape": c.base_shape or "square",
            "mask_shape": c.mask_shape or "square",
            "base_image": c.base_image.url if c.base_image else "",
            "portrait_sample": c.portrait_image.url if c.portrait_image else "",
            "dimension": {
                "id": c.dimension.id if c.dimension else 0,
                "w": float(c.dimension.width) if c.dimension else 1,
                "h": float(c.dimension.height) if c.dimension else 1,
                "label": f"{c.dimension.width} x {c.dimension.height}" if c.dimension else "Default"
            },
            # THESE MUST BE PLURAL
            "thickness": [{"id": t.id, "size": t.size} for t in c.thickness.all()],
            ...
            "image": {"x": float(c.image_x), "y": float(c.image_y), "w": float(c.image_w), "h": float(c.image_h)},
        })
    ...
    if request.method == "POST":
        ...
        for i in range(len(texts)):
            custom = ProductCustomization.objects.create(
                ...
                base_shape=request.POST.get("base_shape"),
                mask_shape=request.POST.get("mask_shape"),

                image_x=request.POST.get("ix"),
                image_y=request.POST.get("iy"),
                image_w=request.POST.get("iw"),
                image_h=request.POST.get("ih"),

            )

            # SAVE MANY TO MANY

            custom.thickness.set(
                request.POST.getlist('thickness')
            )
            ...
            custom.fastening.set(
                request.POST.getlist('fastening')
            )
            ...
            for i in range(len(materials)):
                if materials[i]:
                    ...
                    custom = ProductCustomization.objects.create(
                        ...
                        image_x=ix[i] if ix[i] else 0,
                        image_y=iy[i] if iy[i] else 0,
                        image_w=iw[i] if iw[i] else 0,
                        image_h=ih[i] if ih[i] else 0
                    )

                    custom.material.set([materials[i]])
...
from django.contrib import messages
from .models import Product, PlaqueShape, Thickness, Material, Fastening, Dimension, ProductCustomization


def product_customize(request, pk):
    """
    This view loads the main page.
    It provides the list of all available shapes from the PlaqueShape model.
    """
    product = get_object_or_404(Product, pk=pk)

    context = {
        "product": product,
        # Fetching dynamic shapes from your 'Add Format' model
...
def product_customize_save(request, pk):
    ...
    if request.method == "POST":
        custom = ProductCustomization.objects.create(
            ...
            base_shape=request.POST.get("base_shape", "square"),
            mask_shape=request.POST.get("mask_shape", "square"),
        )

        # SAVING MANY TO MANY FIELDS
        custom.thickness.set(request.POST.getlist('thickness[]'))
        custom.material.set(request.POST.getlist('material[]'))
        custom.fastening.set(request.POST.getlist('fastening[]'))
...
def customization_edit(request, pk):
    # 1. Fetch the customization object and related product
    edit_custom = get_object_or_404(ProductCustomization, pk=pk)
    product = edit_custom.product

    if request.method == "POST":
        # Update basic fields
        edit_custom.dimension_id = request.POST.get("dimension")
        edit_custom.price = request.POST.get("price") or 0
        edit_custom.base_shape = request.POST.get("base_shape")
        edit_custom.mask_shape = request.POST.get("mask_shape")
        edit_custom.image_x = request.POST.get("ix") or 0
        edit_custom.image_y = request.POST.get("iy") or 0
        edit_custom.image_w = request.POST.get("iw") or 0
        edit_custom.image_h = request.POST.get("ih") or 0

        if request.FILES.get("base_image"):
            edit_custom.base_image = request.FILES.get("base_image")

        edit_custom.save()

        # Update Many-to-Many fields
        edit_custom.thickness.set(request.POST.getlist('thickness[]'))
        edit_custom.material.set(request.POST.getlist('material[]'))
        edit_custom.fastening.set(request.POST.getlist('fastening[]'))

        messages.success(request, "Configuration updated successfully!")
        return redirect("product_customize", pk=product.id)

    # 2. Handle GET request: Fetch data needed for the modal dropdowns/checkboxes
    dimensions = Dimension.objects.all()
    thickness = Thickness.objects.all()
    materials = Material.objects.all()
    fastenings = Fastening.objects.all()
    ...
    context = {
        'product': product,
        'edit_custom': edit_custom,  # This triggers the 'isEditMode' in your JS
...
# In your views.py, update the add_to_cart function:
...
def add_to_cart(request):
    if request.method == 'POST':
        ...
        if request.content_type == 'application/json':
            try:
                ...
                cart[cart_key] = {
                    ...
                    'custom_image': data.get('custom_image_base64'),  # The snapshot
                    'quantity': 1,
                    'shape': data.get('shape', 'Standard'),
                    'dimension': data.get('dimension', 'Standard'),
                    'material': data.get('material', 'Standard'),  # Added this
                    'thickness': data.get('thickness', 'Standard'),  # Added this
                    'stand': data.get('stand', 'None'),  # Added this
        ...
        # ... Rest of your existing 'else' logic for standard items ...
...
def checkout(request):
    ...
    # 1. Fetch categories so the header dropdown has data
    categories = Category.objects.all()

    # 2. Treat the dictionary like a list for the template
    ...
    return render(request, 'checkout.html', {
        ...
        'categories': categories  # <--- MUST ADD THIS LINE
    })
...
# def faq(request):
#     # This matches the 'faq' attribute error in your terminal
#     categories = Category.objects.all() # Good for header dropdown
