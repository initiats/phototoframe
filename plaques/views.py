...
def shop(request):
    ...
    # 1. Fetch only customizations belonging to this product
    ...
    for c in customizations:
        configs.append({
            "id": c.id,
            # --- SHAPE FIX: Adding these two lines to pass data to the canvas ---
            "base_shape": c.base_shape ,
            "mask_shape": c.mask_shape ,
            # --------------------------------------------------------------------
            ...
            "text_config": {
                ...
                "label": c.text_value or "Your Inscription"
            }
        })

    # 2. Fetch ALL categories for the nav dropdown
    categories = Category.objects.all()

    # 3. Fetch ALL available options for the selection sidebar
    context = {
        "product": product,
        "categories": categories,
        "shape": PlaqueShape.objects.filter(status=True),
    ...
    if request.method == "POST":
        ...
        for i in range(len(texts)):
            ...
            custom = ProductCustomization.objects.create(
                ...
                image_x=ix[i] if i < len(ix) else 0,
                image_y=iy[i] if i < len(iy) else 0,
                image_w=iw[i] if i < len(iw) else 0,
                image_h=ih[i] if i < len(ih) else 0,

                text_x=tx[i] if i < len(tx) else 0,
                text_y=ty[i] if i < len(ty) else 0,
                text_w=tw[i] if i < len(tw) else 0,
                text_h=th[i] if i < len(th) else 0,
                text_value=texts[i] if i < len(texts) else "",
            )

            # MANY TO MANY FIX
            custom.shape.set(request.POST.getlist("shape[]"))
...
def product_customize_save(request, pk):
    ...
    if request.method == "POST":
        ...
        custom_texts = request.POST.getlist("custom_text[]")
        tx_list = request.POST.getlist("tx[]")
        ty_list = request.POST.getlist("ty[]")
        tw_list = request.POST.getlist("tw[]")
        th_list = request.POST.getlist("th[]")
        ...
        custom = ProductCustomization.objects.create(
            ...
            text_value=custom_texts[0] if custom_texts else "",
            text_x=tx_list[0] if tx_list else 0,
            text_y=ty_list[0] if ty_list else 0,
            text_w=tw_list[0] if tw_list else 0,
            text_h=th_list[0] if th_list else 0,
        ...
        # ✅ MANY TO MANY FIX (VERY IMPORTANT)
        custom.shape.set(request.POST.getlist("shape[]"))
        custom.thickness.set(request.POST.getlist("thickness[]"))
        custom.material.set(request.POST.getlist("material[]"))
        custom.fastening.set(request.POST.getlist("fastening[]"))
...
def customization_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        custom.base_shape = request.POST.get("base_shape", "square")
        custom.mask_shape = request.POST.get("mask_shape", "square")
        ...
        if texts:
            ...
            custom.text_x = tx_list[0] if tx_list else 0
            custom.text_y = ty_list[0] if ty_list else 0
            custom.text_w = tw_list[0] if tw_list else 0
            custom.text_h = th_list[0] if th_list else 0
    ...
    # Context for the Edit mode (includes dynamic shapes)
    context = {
        "product": product,
        "edit_custom": custom,
        "shapes": PlaqueShape.objects.filter(status=True),  # Added here too
        "dimensions": Dimension.objects.all(),
        "thickness": Thickness.objects.filter(status=True),
        "materials": Material.objects.filter(status=True),
        "fastenings": Fastening.objects.filter(status=True),
